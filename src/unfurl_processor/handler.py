...
def fetch_instagram_data_with_browser(url: str) -> Optional[Dict[str, Any]]:
    """
    Fetch Instagram data using browser automation (Playwright).

    This method is used as a fallback when regular HTTP requests fail.
    It uses a real browser instance to bypass more sophisticated bot detection.
    """
    ...
    try:
        ...
        logger.info(
            "Attempting browser automation for Instagram", extra={"url": canonical_url}
        )
        ...
        with sync_playwright() as p:
            ...
            # Navigate with random human-like delay
            time.sleep(random.uniform(1.0, 3.0))  # nosec B311

            try:
                response = page.goto(
                    canonical_url, wait_until="domcontentloaded", timeout=30000
                )

                if response and response.status == 200:
                    # Wait for content to load
                    page.wait_for_timeout(random.uniform(2000, 4000))  # nosec B311
                    ...
                    logger.warning(
                        "Browser automation got non-200 response",
                        extra={
                            "url": canonical_url,
        ...
        logger.error(
            "Browser automation failed", extra={"error": str(e), "url": canonical_url}
        )
        if metrics:
            metrics.add_metric(
                name="BrowserAutomationError", unit=MetricUnit.Count, value=1
            )
